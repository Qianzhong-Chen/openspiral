...
Variant = Literal["dummy", "gemma_300m", "gemma_2b", "gemma_2b_freeze", "gemma_2b_lora", "gemma_300m_lora", "gemma_2b_lora_rank_32"]
...
def get_config(variant: Variant) -> Config:
    ...
    if variant == "gemma_2b_lora_rank_32":
        return Config(
            width=2048,
            depth=18,
            mlp_dim=16_384,
...
class Module(nn.Module):
    ...
    def __call__(
        ...
        *,
        ...
        return [f(e, a)[0] if e is not None else e for f, e, a in zip(self.final_norms, embedded, adarms_cond, strict=True)], kv_cache

    def init(self, use_adarms=None):
        """Convenience method for initializing all parameters, necessary due to the quirks of linen."""
        if use_adarms is None:
            use_adarms = [False] * len(self.configs)
        self.embed(jnp.zeros((1, 1), dtype=jnp.int32))
        adarms_cond = [
            jnp.zeros((1, c.width)) if use else None
            for c, use in zip(self.configs, use_adarms)
        ]
        self(
