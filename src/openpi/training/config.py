...
class LeRobotXmiRbyDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[
                xmi_rby_policy.XmiRbyInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)
            ],
        ...
        delta_action_mask = _transforms.make_bool_mask(
            6,
            3,
            -1,  # left: 6d_rot (delta), 3d_pos (delta), gripper (absolute)
            6,
            3,
            -1,  # right: 6d_rot (delta), 3d_pos (delta), gripper (absolute)
        )
        data_transforms = data_transforms.push(
            inputs=[_transforms.DeltaActions(delta_action_mask)],
            outputs=[_transforms.AbsoluteActions(delta_action_mask)],
...
class LeRobotYamDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # YAM data has absolute joint actions, but pi05 pretraining used delta actions.
        # Convert to delta (relative to current state) for joints, keep grippers absolute.
        if self.action_space == "joint":
            delta_action_mask = _transforms.make_bool_mask(6, -1, 6, -1)  # [left_6_joints, left_gripper, right_6_joints, right_gripper]
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaActions(delta_action_mask)],
                outputs=[_transforms.AbsoluteActions(delta_action_mask)],
            )
...
class TrainConfig:
    ...
    # Project name.
    project_name: str = "openpi_tshirt_folding_rl" # "openpi_tshirt_folding"
    ...
    # Using reward model to conduct weighted BC
    rm_weighted: bool = False
    rm_weight_method: str = "binary"
...
_CONFIGS = [
    ...
    TrainConfig( # BC Finetune Config 
        name="pi05_yam_clean_whiteboard",
        ...
        ema_decay=None,
        batch_size=32,
        project_name="openpi_clean_whiteboard",
    ),
    
    
    TrainConfig( # Pi model serving config for RL finetune
        name="residual_rl_pi05_clean_whiteboard",
        model=pi0_config.Pi0Config(pi05=True, 
                                   paligemma_variant="gemma_2b_lora", 
                                   action_expert_variant="gemma_300m_lora", 
                                   action_horizon=50, 
...
# Residual RL configs
# ---------------------------------------------------------------------------
# Separate registry for RL training configs used by scripts/train_residual_rl.py.

def _make_rl_configs():
    from openpi.models.residual_rl_model import ResidualRLConfig

    @dataclasses.dataclass
    class ResidualRLTrainConfig:
        """Full config for offline residual RL training."""
        name: str = ""

        # Which TrainConfig to use for the frozen pi0 model + dataset.
        pi0_config_name: str = "residual_rl_pi05_clean_whiteboard"
        pi0_checkpoint: str = ""

        # RL model hyperparameters.
        rl: ResidualRLConfig = dataclasses.field(default_factory=ResidualRLConfig)
        ...
        checkpoint_dir: str = "./SPIRAL_ckpt"
        project_name: str = "openpi-residual-rl"
        exp_name: str = "spiral_policy"
        wandb_enabled: bool = True

        # Misc.
        critic_updates_per_step: int = 1
        pi0_num_steps: int = 10

        # MC return calculation.
        # If True, reads a per-frame "progress" column from the dataset and uses the
        # last frame's progress of each episode as the terminal reward scale for the
        # MC return. If False, assumes terminal reward = 1.0 for every episode.
        use_progress_mc_return: bool = False

    return ResidualRLTrainConfig, [
        
        ResidualRLTrainConfig(
            name="residual_rl_clean_whiteboard_spiral",
            pi0_config_name="residual_rl_pi05_clean_whiteboard",
            wandb_enabled=True,
            pi0_checkpoint="<PATH_TO_BASE_Pi_CKPT/params>",
            rl=ResidualRLConfig(
                beta_bc=30.0,
                num_noise_samples=4,
                num_critics=5,
        ...
        ResidualRLTrainConfig(
            name="residual_rl_clean_whiteboard_sparse",
            pi0_config_name="residual_rl_pi05_clean_whiteboard",
            wandb_enabled=True,
            pi0_checkpoint="<PATH_TO_BASE_Pi_CKPT/params>",
            rl=ResidualRLConfig(
        ...
        ResidualRLTrainConfig(
            name="residual_rl_clean_whiteboard_dense_only",
            pi0_config_name="residual_rl_pi05_clean_whiteboard",
            wandb_enabled=True,
            pi0_checkpoint="<PATH_TO_BASE_Pi_CKPT/params>",
            rl=ResidualRLConfig(
...
_RL_CONFIG_CLASS, _RL_CONFIGS = _make_rl_configs()

if len({c.name for c in _RL_CONFIGS}) != len(_RL_CONFIGS):
    raise ValueError("RL config names must be unique.")
_RL_CONFIGS_DICT = {c.name: c for c in _RL_CONFIGS}


def get_rl_config(config_name: str):
    """Get an RL training config by name."""
    if config_name not in _RL_CONFIGS_DICT:
        closest = difflib.get_close_matches(config_name, _RL_CONFIGS_DICT.keys(), n=1, cutoff=0.0)
        closest_str = f" Did you mean '{closest[0]}'? " if closest else ""
        raise ValueError(f"RL config '{config_name}' not found.{closest_str}")
    return _RL_CONFIGS_DICT[config_name]


def rl_cli():
    """CLI entry point for RL training configs (used by scripts/train_residual_rl.py)."""
    return tyro.extras.overridable_config_cli({k: (k, v) for k, v in _RL_CONFIGS_DICT.items()})
