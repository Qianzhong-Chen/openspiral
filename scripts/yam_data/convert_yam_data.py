...
class YAMSConfig:
    ...
    max_workers: int = 2  # Set lower on machines with less memory -- must be 1 for cartesian
    ...
    max_frames_per_chunk: int = 1000  # TODO: Process episodes in chunks to avoid OOM on long episodes
...
def process_yam_episode(
    ...
    # TODO: temporary fix if new dataset has no metadata
    # task_name = extract_task_name_from_episode(episode_data, episode_path)
    task_name = cfg.language_instruction
    ...
    if not records:
        ...
        return None
    elif type(records) == str and records == "error":
        raise ValueError(f"joint state issue in episode {episode_path}")
